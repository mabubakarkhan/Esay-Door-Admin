                        ...
                        <div class="col-md-12">
                            <div class="card">
                                ...
                                <div class="card-content">
                                    ...
                                    <a class="pull-right" style="" data-toggle="modal" href='#modal-add'>
                                    ...
                                    <div class="material-datatables">
                                        <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                                            ...
                                            <tbody>
                                                <?php foreach($colors as $q){ ?>
                                                <tr>
                                                    <td><?=$q['name']?></td>
                                                    <td><?=$q['full_name']?></td>
                                                    <td class="td-actions text-right">
                                                        <button type="button" rel="tooltip" class="btn btn-success btn-round edit-color" data-id="<?=$q['color_id']?>" data-name="<?=$q['name']?>" data-font="<?=$q['font_color']?>">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.edit-color', function(event) {
            ...
            $this = $(this);
            $("#modal-edit input[name='color_id']").val($this.attr('data-id'));
            $("#modal-edit input[name='name']").val($this.attr('data-name'));
            $("#modal-edit input[name='font_color']").val($this.attr('data-font'));
        ...
        $(document).on('submit', '.edit-form', function(event) {
            event.preventDefault();
            $form = $(this);
            $.post("<?=site_url('admin/update_color')?>", {data: $form.serialize()}, function(resp) {
                resp = JSON.parse(resp);
                if (resp.status == true) {
                    alert('updated :) will display after reload page');
                    $("#modal-edit").modal('hide');
                }
                else{
                    alert('not updated :( please try again or reload your web page');
                }
            });
        });
        ...
        $(document).on('submit', '.add-form', function(event) {
            event.preventDefault();
            $form = $(this);
            $.post("<?=site_url('admin/post_color')?>", {data: $form.serialize()}, function(resp) {
                resp = JSON.parse(resp);
                if (resp.status == true) {
                    alert('added :) will display after reload page');
                    $("#modal-add").modal('hide');
                }
                else{
                    alert('not added :( please try again or reload your web page');
                }
            });
        });
            ...
            <div class="modal-body">
                ...
                <form class="edit-form">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="font_color" class="form-control" required="required" value="">
                    </div><!-- /form-group -->
                    <div class="form-group">
